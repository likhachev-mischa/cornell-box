...
bool ObjLoader::loadMtl(const std::string& path, Material& material) {
    std::ifstream file(path);
    if (!file.is_open()) {
        std::cerr << "Error: Could not open material file " << path << std::endl;
        return false;
    }
    
    // Extract the directory path to use for relative texture paths
    ...
    while (std::getline(file, line)) {
        ...
        if (prefix == "Ka") {
            // Ambient color
            iss >> material.ambient_color.r >> material.ambient_color.g >> material.ambient_color.b;
        } 
        else if (prefix == "Kd") {
            // Diffuse color
            iss >> material.diffuse_color.r >> material.diffuse_color.g >> material.diffuse_color.b;
        } 
        else if (prefix == "Ks") {
            // Specular color
            iss >> material.specular_color.r >> material.specular_color.g >> material.specular_color.b;
        } 
        else if (prefix == "Ns") {
            // Shininess
            ...
            // Handle relative paths by prepending the directory
            if (!texturePath.empty() && texturePath[0] != '/' && directory != "") {
                material.diffuse_texture_path = directory + texturePath;
            } else {
